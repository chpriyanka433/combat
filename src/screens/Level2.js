 ...
 const totalQuestion = total
  ...
  const handleNextQuestion = () => {
    if (answers[currentQuestion] === null) {
      ...
      console.log("Yes count level2:", yesCount);
console.log("No count level2:", noCount);
        Alert.alert('Level 2 Questions Complete')
        navigation.navigate('level31')
        if(yesCount>8){
          AsyncStorage.setItem("resultlevel2", "Highly");
          AsyncStorage.setItem("levelStatus", "Completed");
        }
        else if(yesCount>3){
          AsyncStorage.setItem("resultlevel2", "Medium level");
        }
        else{
    ...
    <View style={styles.container}>
      <SafeAreaView>
        ...
        <TouchableOpacity style={styles.summaryButton} onPress={testSummary}>
          <Text style={{ color: '#ffffff', fontWeight: 'bold',marginTop:6 }}>TEST SUMMARY</Text>
        </TouchableOpacity>
        ...
        <View style={{ backgroundColor: '#2843de', height: hp(4), width: wp(35), alignSelf: 'center',  alignItems: 'center', marginTop: 10, marginBottom: 10, borderRadius: 5 }}>
          <Text style={{ fontSize: 14, fontWeight: 'bold', color: '#ffffff',marginTop:6 }}>Level- 2.0</Text>
        </View>
